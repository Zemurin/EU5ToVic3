echo on
rem Copy converter data files
copy "Data_Files\log.txt" "..\Release\EU5ToVic3\"
copy "Data_Files\configuration-example.txt" "..\Release\EU5ToVic3\"
copy "Resources\rakaly.dll" "..\Release\EU5ToVic3\"

mkdir "..\Release\Configuration"
copy "Data_Files\fronter-configuration.txt" "..\Release\Configuration\"
copy "Data_Files\fronter-options.txt" "..\Release\Configuration\"
copy "Data_Files\*.yml" "..\Release\Configuration\"

mkdir "..\Release\Docs"
copy "Data_Files\ReadMe.txt" "..\Release\Docs\"
copy "Data_Files\FAQ.txt" "..\Release\Docs\"
copy "Data_Files\license.txt" "..\Release\Docs\"
copy "Data_Files\after_converting.txt" "..\Release\Docs\"

rem Create Configurables
del "..\Release\EU5ToVic3\configurables" /Q
rmdir "..\Release\EU5ToVic3\configurables" /S /Q
xcopy "Data_Files\configurables" "..\Release\EU5ToVic3\configurables" /Y /E /I

rem Create Blank Mod
del "..\Release\EU5ToVic3\blankMod" /Q
rmdir "..\Release\EU5ToVic3\blankMod" /S /Q
xcopy "Data_Files\blankMod" "..\Release\EU5ToVic3\blankMod" /Y /E /I
